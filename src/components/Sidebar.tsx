...
import { ArrowUp, ArrowDown, Trash2, Plus, ChevronLeft, Palette, Type, ScanLine, CircleDashed } from 'lucide-react';
...
// ... preserving rest of functions ...
  ...
  const addLine = () => {
    ...
    updateState({
      lines: [
        ...state.lines,
        { id: Math.random().toString(), text: 'New Line', font: FONTS[0].url, size: 8, depth: 1.5 }
      ]
    });
  };
    ...
    <aside className="sidebar">
      ...
      <div className="sidebar-content">
        ...
        <div className="control-group">
          <h2 className="control-title" style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><Type size={14}/> Lines ({state.lines.length} of 8)</h2>
          
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {state.lines.map((line, idx) => (
              <div key={line.id} className="line-card">
                <div className="line-card-header">
                  <span className="line-card-title">Line {idx + 1}</span>
                  <div className="line-card-actions">
                    <button className="action-btn" onClick={() => moveLine(idx, 'up')} disabled={idx === 0}><ArrowUp size={14}/></button>
                    <button className="action-btn" onClick={() => moveLine(idx, 'down')} disabled={idx === state.lines.length - 1}><ArrowDown size={14}/></button>
                    <button className="action-btn danger" onClick={() => removeLine(idx)} disabled={state.lines.length === 1}><Trash2 size={14}/></button>
                  </div>
                </div>

                <div className="control-item">
                  <input type="text" value={line.text} onChange={e => updateLine(idx, { text: e.target.value })} placeholder="Content" />
                </div>
                
                <div className="control-item">
                  <select 
                    value={line.font} 
                    onChange={e => updateLine(idx, { font: e.target.value })}
                    style={{ fontFamily: FONTS.find(f => f.url === line.font)?.name || 'inherit', fontSize: '1rem', padding: '6px 12px' }}
                  >
                    {FONTS.map(f => <option key={f.url} value={f.url} style={{ fontFamily: f.name, fontSize: '1.1rem' }}>{f.name}</option>)}
                  </select>
                </div>
          ...
          <button onClick={addLine} disabled={state.lines.length >= 8} className="btn-outline" style={{ marginTop: '4px', borderStyle: 'dashed', padding: '12px' }}>
            <Plus size={18} style={{marginRight:'6px'}}/> Add line
          </button>
        ...
        <div className="control-group">
          <h2 className="control-title" style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><ScanLine size={14}/> Shape</h2>

          <div style={{ marginTop: '4px' }}>
            <span style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--text-tertiary)', textTransform: 'uppercase' }}>Size</span>
            <div style={{ marginTop: '12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span className="hud-label" style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-secondary)' }}>Auto Size</span>
                <input 
                  type="checkbox" 
                  ...
                  onChange={e => {
                    ...
                    if (isAuto) {
                      updateState({ shape: { ...state.shape, autoSize: true } });
                    } else {
                      updateState({ shape: { ...state.shape, autoSize: false, width: bounds.x, height: bounds.y } });
                    }
                  }} 
                />
              </div>
              ...
              <div className="line-grid">
                <div className="control-item">
                  <label><span>Width</span><span className="control-value">{state.shape.width.toFixed(1)}</span></label>
                  <input type="range" min={20} max={300} step={1} value={state.shape.width} onChange={e => updateState({ shape: { ...state.shape, width: parseFloat(e.target.value) } })}/>
                </div>
                <div className="control-item">
                  <label><span>Height</span><span className="control-value">{state.shape.height.toFixed(1)}</span></label>
                  <input type="range" min={10} max={200} step={1} value={state.shape.height} onChange={e => updateState({ shape: { ...state.shape, height: parseFloat(e.target.value) } })}/>
                </div>
              </div>

              <div className="line-grid" style={{ marginTop: '12px' }}>
                <div className="control-item">
                  <label><span>Corner radius</span><span className="control-value">{state.shape.cornerRadius.toFixed(1)}</span></label>
                  <input type="range" min={0} max={50} step={0.5} value={state.shape.cornerRadius} onChange={e => updateState({ shape: { ...state.shape, cornerRadius: parseFloat(e.target.value) } })}/>
                </div>
                <div className="control-item">
                  <label><span>Inner radius</span><span className="control-value">{(state.shape.innerRadius ?? 20).toFixed(1)}</span></label>
                  <input type="range" min={0} max={50} step={0.5} value={state.shape.innerRadius ?? 20} onChange={e => updateState({ shape: { ...state.shape, innerRadius: parseFloat(e.target.value) } })}/>
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={{ marginTop: '8px' }}>
            <span style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--text-tertiary)', textTransform: 'uppercase' }}>Depth</span>
            <div className="line-grid" style={{ marginTop: '12px' }}>
              <div className="control-item">
                <label><span>Base</span><span className="control-value">{state.shape.baseThickness.toFixed(1)}</span></label>
                <input type="range" min={0.5} max={10} step={0.1} value={state.shape.baseThickness} onChange={e => updateState({ shape: { ...state.shape, baseThickness: parseFloat(e.target.value) } })}/>
              </div>
              <div className="control-item">
                <label><span>Top border</span><span className="control-value">{state.shape.topBorder.toFixed(1)}</span></label>
                <input type="range" min={0} max={10} step={0.1} value={state.shape.topBorder} onChange={e => updateState({ shape: { ...state.shape, topBorder: parseFloat(e.target.value) } })}/>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="control-group">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h2 className="control-title" style={{ margin: 0, display: 'flex', alignItems: 'center', gap: '6px' }}><CircleDashed size={14}/> Lace hole</h2>
            <input 
              type="checkbox" 
              checked={state.laceHole.enabled} 
              onChange={e => updateState({ laceHole: { ...state.laceHole, enabled: e.target.checked } })} 
            />
          </div>
          ...
          {state.laceHole.enabled && (
            <div className="line-grid" style={{ marginTop: '8px' }}>
              ...
              <div className="control-item" style={{ gridColumn: 'span 2' }}>
                <label><span>Top margin</span><span className="control-value">{state.laceHole.topMargin.toFixed(1)}</span></label>
                <input type="range" min={0} max={20} step={0.5} value={state.laceHole.topMargin} onChange={e => updateState({ laceHole: { ...state.laceHole, topMargin: parseFloat(e.target.value) } })}/>
              </div>
            </div>
          )}
        </div>
